<template>
  <div class="min-h-screen bg-bg" dir="rtl">
    ...
    <main class="p-6 max-w-5xl mx-auto">
      ...
      <!-- Bulk action bar -->
      <div v-if="canManage && filtered.length > 0" class="flex items-center gap-3 mb-5">
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div v-for="b in filtered" :key="b.id"
          ...
          :class="selected.has(b.id) ? 'border-blue/40 ring-2 ring-blue/20 bg-blue/5' : 'border-border'"
          @click="canManage && toggleSelect(b.id)">
          <div class="flex justify-between items-start">
            <div class="flex items-start gap-2 flex-1">
              <span v-if="canManage" class="mt-0.5 w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors"
                :class="selected.has(b.id) ? 'bg-blue border-blue' : 'border-gray-300'">
                <svg v-if="selected.has(b.id)" class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/>
                </svg>
              </span>
              <div>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" @click.self="showModal = false">
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 overflow-y-auto max-h-[90vh]" @click.stop>
          ...
          <div class="space-y-3">
            ...
            <div>
              <label class="block text-xs font-medium text-text-light mb-1">اسم الشركة <span class="text-text-light/60 font-normal">(اختياري)</span></label>
            ...
            <div class="grid grid-cols-2 gap-2">
              <div>
                <label class="block text-xs font-medium text-text-light mb-1">السجل التجاري <span class="text-text-light/60 font-normal">(اختياري)</span></label>
              ...
              <div>
                <label class="block text-xs font-medium text-text-light mb-1">المدينة <span class="text-text-light/60 font-normal">(اختياري)</span></label>
...
const allSelected = computed(() => filtered.value.length > 0 && filtered.value.every(b => selected.value.has(b.id)))
function toggleSelect(id: string) {
  const s = new Set(selected.value); s.has(id) ? s.delete(id) : s.add(id); selected.value = s
}
function toggleSelectAll() {
  allSelected.value ? selected.value = new Set() : selected.value = new Set(filtered.value.map(b => b.id))
}
async function deleteSelected() {
  if (!confirm(`حذف ${selected.value.size} وسيط؟`)) return
  try {
    await Promise.all([...selected.value].map(id => brokersApi.remove(id)))
